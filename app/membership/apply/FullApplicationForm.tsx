...
// The full membership application — the long path from /membership/apply.
//
// Everything here supports the admin approve/decline decision at
// /admin/applications: the plan selection prices the subscription, the
// references and emergency contact inform the decision itself. That is why
// it is long, and why it stays long. Someone on this path has already
// decided to pay $200–$1,200 a month; the questions are proportionate.
//
// Trial-day applicants no longer see any of it. They take
// TrialApplicationForm instead, and arrive here later — if they arrive at
// all — through a resume link that prefills what they already gave us.
...
interface MembershipApplication {
  ...
  start_date: string;
  // Always false on this path — a trial day is its own form now. Kept on
  // the payload because the API and admin panel still read it.
...
interface FullApplicationFormProps {
  // Details carried over from a trial day, when the applicant arrived on a
  // resume link. Null for someone applying directly.
  prefill?: TrialPrefill | null;
  resumeToken?: string | null;
  // Returns to the path chooser on /membership/apply.
  onChangePath?: () => void;
}
...
export default function FullApplicationForm({
  ...
  // Fill in what a returning trial applicant already told us. Everything
  // stays editable — someone converting weeks after their visit may have
  // changed jobs or numbers, and a locked field they cannot correct is worse
  // than an empty one. `wants_trial_day` is false on this path either way:
  // the trial/membership choice was made on the chooser, and a returning
  // trial applicant has already had their day.
  useEffect(() => {
    if (!prefill) return;
    setApplication(prev => ({
      ...prev,
  ...
  // Preselect the plan they came in to try, so the commonest case is one
  ...
  // page's "Apply" button, for instance — lands on /membership/apply with
  // ?plan=<id>. The chooser routes that straight here; this pre-selects the
  // plan so the visitor is not dropped into a six-option picker having
  // already chosen. Read off window.location rather than useSearchParams,
  // which would force a Suspense boundary and cost the page its static
  // render.
    ...
    try {
      ...
      const response = await fetch('/api/membership-application', {
        ...
        body: JSON.stringify({
          ...
          // Ties this submission back to the trial day it came from, so the
          // follow-up emails stop and the photo ID carries across.
      ...
      <section className="bg-linen py-16">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h1 className="font-display text-4xl md:text-5xl font-semibold text-ink mb-6">
            {prefill ? 'Finish Your Membership Application' : 'Apply for Membership'}
          </h1>
          <p className="text-xl text-ink-60 mb-8">
            {prefill
              ? 'Everything you gave us for your trial day is already filled in below.'
              : "Join our community of professionals in the heart of Sloan's Lake. Complete your application below to get started."}
          </p>
            ...
            <div className="mx-auto mb-8 max-w-2xl border border-accent/40 bg-bone p-4 text-left">
              <p className="text-[15px] leading-relaxed text-ink">
                <span className="font-semibold">
                  Welcome back{prefill.first_name ? `, ${prefill.first_name}` : ''}.
                </span>{' '}
                Your name, contact details{prefill.company_name ? ', company' : ''} and the
                space you tried are already filled in &mdash; check them over and change
                anything that has moved on.
                {prefill.has_id_document && ' Your photo ID is on file from your trial day, so there is nothing to upload again.'}
              </p>
            </div>
          )}

          {!prefill && onChangePath && (
            <button
              type="button"
          ...
          <div className="flex items-center justify-center gap-4 text-sm text-ink-60">
            <div className="flex items-center gap-2">
              ...
              <span>{prefill ? 'Trial day complete' : 'Free trial day available'}</span>
            </div>
            <div className="flex items-center gap-2">
              <CheckCircle className="w-4 h-4 text-accent" />
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <form onSubmit={handleSubmitApplication} className="space-y-8">
            ...
            <div className="bg-bone border border-clay p-6">
              ...
              {/* The trial/membership choice happened on the chooser, so there
                  is no radio here any more. This is the escape hatch for
                  someone who got this far and would rather try a day first —
                  cheaper to offer it than to lose them to the form length. */}
              {!prefill && onChangePath && (
                <div className="mt-6 border border-clay bg-linen p-4">
                  <p className="text-[15px] leading-relaxed text-ink">
                    <span className="font-semibold">Would you rather try a day first?</span>{' '}
                    The trial day is free and asks for far less than this form.{' '}
                    <button
                      type="button"
                      onClick={onChangePath}
                      className="font-semibold text-accent underline underline-offset-2 hover:text-accent-deep"
                    >
                      Book a trial day instead
                    </button>
                    .
                  </p>
                </div>
