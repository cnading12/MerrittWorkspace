...
/**
 * `lastModified` used to be `new Date()` on every entry, which told crawlers
 * that all thirteen pages changed every time the site was deployed. Crawlers
 * learn to distrust a sitemap that cries wolf, and then ignore the dates that
 * are true. These are the real dates the pages last changed in substance; bump
 * the one you edited when you edit it, and leave the rest alone.
 *
